...
public class BoardBehavior : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (currentBeat > 0)
        {
            transform.position = Vector3.Lerp(lastPos, targetPos, count * ( LevelManager.Instance.level.soTrack.bpm /60));
        }
    }
    ...
    void UpdateMovementTarget()
    {
        ...
        Vector3 tmpos;
        tmpos = Vector3.back * LevelManager.Instance.spawnDistance;
        Debug.Log(originPos);
        targetPos = lastPos + (Vector3.back * LevelManager.Instance.spawnDistance / LevelManager.Instance.beatsToPlayer) * currentBeat;
        //Debug.Log(count);
