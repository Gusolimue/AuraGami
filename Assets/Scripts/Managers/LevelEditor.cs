...
public class LevelEditor : MonoBehaviour
{
    ...
    [SerializeField, HorizontalGroup(true, nameof(saveButtonHolder), nameof(loadButtonHolder))] 
    private VoidStructure sLbuttonGroupHolder;
    [ButtonField(nameof(SaveLevel), "Save Level", 30f)]                          
    [SerializeField, HideInInspector] VoidStructure saveButtonHolder;
    [ButtonField(nameof(LoadLevel), "Load Level", 30f)]                          
    [SerializeField, HideInInspector] VoidStructure loadButtonHolder;
    void SaveLevel()
    {
        for (int c = 1; c < 4; c++)
        {
            ...
            {
                if (level.GetStage(c).Count > i)
                {
                    level.GetStage(c)[i] = new Board(GetStage(c)[i].interactables);
                }
            }
        }
    }
    void LoadLevel()
    {
        ...
        {
            GetStage(c).Clear();
            for (int i = 0; i < level.GetStage(c).Count; i++)
            {
                GetStage(c).Add(new Board(level.GetStage(c)[i].interactables));
            }
        }
    }
    [Header("Stages")]
    ...
    //void Redo(){}
    [SerializeField, HideInInspector] GameObject[] threadedTargets;
    [FoldoutGroup("Advanced Tools", nameof(setStageListButtonHolder), nameof(setTestLayoutButtonHolder), nameof(iWantToDeleteMyStagePermanently), nameof(levelToPasteTo), nameof(pasteToSoLevelButtonHolder), nameof(threadedTargets))]
